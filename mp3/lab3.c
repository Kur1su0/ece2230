 ...
 *
 * Propose: The aim of lab3 is to use linked-list to sort CSRB imformation.
 *
...
int main(int argc, char * argv[])
{
    ...
    //printf("CREATE x; ADDSU; DELSU x\nLISTCH x; CLEARCH x; ADDTAIL su_id ");
    //printf("SORT x; CHANGECH old new; ASSIGNSU x\nPRINTASSIGNED; PRINTWAITING\n");
    //printf("STATS; QUIT\n");
    ...
    while (fgets(line, MAXLINE, stdin) != NULL) {
        ...
        if (num_items == 2 && strcmp(command, "CREATE") == 0) {
            ...
            int su_id = input_1;
            sas_addtail( waiting_list,  su_id);

        } else if (num_items == 2 && strcmp(command, "SORT") == 0) {
            int type = input_1;
            if (type > 4 || type < 1){
            }else{
                 sas_list_sort(waiting_list, type);
            }
