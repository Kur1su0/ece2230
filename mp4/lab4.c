...
int main(int argc, char **argv)
{
    ...
    //      fragments at head of the free list
        ...
        printf("\nTrial 2: Print Free list.\n");
        Mem_print();



        printf("\nTrial 3: free p1 and p3\n");
        printf("Trial 4: verify hole between p1 & p3 \n");
        ...
        //Check if the new 1/4 pages will be alloced.

        printf("\nTrial 5: requests a hold page\n");
        //Trail request a whole page.
        ...
        printf("\nTrail 6: free all block back to free list.");
    ...
    else if (dprms.UnitDriver == 3)
    {
        ...
        // allocate 1st pointer to 1/2 of a page
        num_bytes_1 = (units_in_first_page/2 - 1)*unit_size;
        p1 = (int *) Mem_alloc(num_bytes_1);
        printf("first: %d bytes (%d units) p=%p \n",
                num_bytes_1, num_bytes_1/unit_size, p1);
        Mem_print();
        ...
        printf("first free of 1/2 a page p=%p \n", p1);
        Mem_free(p1);
        Mem_print();



        printf("third free of 1/4 a page p=%p \n", p3);
        Mem_free(p3);
        Mem_print();
        ...
        printf("\nTESTI 1: BEST_FIT POLICY performance\n");
        num_bytes_4 = (units_in_first_page/4 - 1)*unit_size;
        p4 = (int *) Mem_alloc(num_bytes_4);
        printf("second: %d bytes (%d units) p=%p \n",
                num_bytes_4, num_bytes_4/unit_size, p4);
